...
function speciesCoordinates(data) {
  // TODO
  //  loop over results array which contains objects of observation
  //console log the "species_guess" observed at coordinates "location"
...
function observationsByQualityGrade(data, qualityGrade) {
  // TODO
  //If quality grade is a string, convert to lowercase to handle non-lowercase passed in
  if (typeof qualityGrade === 'string') qualityGrade = qualityGrade.toLowerCase();
  //If it's not any of these throw an error
  if (
  ...
  //If observation data quality grade is the same as what's passed in, push into array
  let matches = [];
  for (let i = 0; i < data.results.length; i++) {
    if (data.results[i].quality_grade === qualityGrade) {
...
function observationsByQualityGrades(data, ...qualityGrades) {
  ...
  let combined = [];
  //Iterate over all quality grades passed in
  for (let i = 0; i < qualityGrades.length; i++) {
    //Assign a current quality grade
    let qualityGrade = qualityGrades[i];
    //Pass to singular quality grade function
    let matches = observationsByQualityGrade(data, qualityGrade);
    //Concatenates all the elements together in a single flat array
    combined = combined.concat(matches);
  }
...
function transformObservation(original) {
  ...
  newObject.name = original.species_guess.toLowerCase();
  if (original.conservation_status) {
    newObject.isExtinct = original.conservation_status.status_name === 'extinct in the wild';
  } else {
    newObject.isExtinct = false;
  }
  newObject.images = original.photos.map(function (photo) {
    return {
      url: photo.url,
      copyright: photo.attribution
    };
  });
  newObject.observer = `${original.user.login_exact}@inaturalist.com`;
  return newObject;
}
...
function transformObservations(data) {
  ...
  let transformed = [];
  for (let i = 0; i < data.results.length; i++) {
    transformed.push(transformObservation(data.results[i]));
  }
  return transformed;
}
...
function transformObservations2(data) {
  // TODO
  //Practicing arrow notation
  //Parameter  =>  Return
  let newArr = data.results.map((observation) => transformObservation(observation));
  return newArr;
}
...
function getObservationsByTaxa(data, ...taxaNames) {
  let matches = [];
  //For each taxon in the list of taxa
  taxaNames.forEach(function (taxon) {
    //Filter out our taxa list with the taxon that matches
    let matchingObservations = data.results.filter(function (observation) {
      return observation.taxon.iconic_taxon_name === taxon;
    });
    //Appends each element of matchingObservations to the end of matches
    matches.push(...matchingObservations);
  });
  //If there was only 1 match, return only that match
  let result = matches.length === 1 ? [matches[0]] : matches;
  return result;
}
...
function getObservationsByLocation(data, options = {}) {
  // TODO
  //If no coordinates provided, return array as is
  if (!options.lat && !options.lng) {
    return data.results;
  }
  //If coordinates are provided, continue
  let filteredArr = data.results.filter(function (observation) {
    //Split our location data into variables
    let [ourLatString, ourLongString] = observation.location.split(',');
    //Convert to float
    let floatLat = parseFloat(ourLatString);
    let floatLong = parseFloat(ourLongString);
    //If there is no min or max passed in, return exact matches to filtered array
    if (!options.lat.min && !options.lng.min) {
      return floatLat === options.lat && floatLong === options.lng;
    }
    //If there is a min or max passed in, return matches that are in range
    return (
      floatLat >= options.lat.min &&
      floatLat <= options.lat.max &&
      floatLong >= options.lng.min &&
      floatLong <= options.lng.max
    );
  });
  return filteredArr;
}
...
function getPlaceURLs(data) {
  ...
  data.results.forEach(function (observation) {
    //and for every element in place_ids array
    for (let i = 0; i < observation.place_ids.length; i++) {
      //Convert the ID to an array
      let stringID = observation.place_ids[i].toString();
      //Push it into our array prefixed by URL
      urlArr.push('https://www.inaturalist.org/observations?place_id=' + stringID);
    }
  });
  return urlArr;
}
...
function getSpeciesObservations(data) {
  ...
  //For each observation
  data.results.forEach(function (observation) {
    //Assign taxon name as key name for our object
    let speciesName = observation.taxon.name;
    //If the species name is in our object
    if (speciesObject[speciesName]) {
      //Increment it
      speciesObject[speciesName]++;
    } else {
      //Assign the count to one of our new species name
...
function extractSpeciesNames(data) {
  ...
  //For each observation
  data.results.forEach(function (observation) {
    //If the array doesn't have the species guess property
    if (!names.includes(observation.species_guess)) {
      //Push it into the array
      names.push(observation.species_guess);
    }
  });
  return names;
}
...
function extractSpeciesNames2(data) {
  ...
  // For each observation
  data.results.forEach(function (observation) {
    // If the set doesn't have the species guess property
    if (!namesSet.has(observation.species_guess)) {
      // Add it to the set
      namesSet.add(observation.species_guess);
    }
  });
